-- FINAL SECURITY FIX - Drop and recreate all functions with security
...
CREATE FUNCTION get_current_usage(user_uuid UUID)
RETURNS TABLE (
...
CREATE FUNCTION log_preference_changes()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = ''
AS $$
BEGIN
    INSERT INTO public.user_activity (
        user_id, activity_type, entity_type, description, metadata
    ) VALUES (
        NEW.user_id, 'preferences_updated', 'settings',
...
CREATE FUNCTION auto_create_calendar_reminders()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = ''
AS $$
...
BEGIN
    SELECT * INTO user_prefs FROM public.user_preferences WHERE user_id = NEW.user_id;
    
    IF user_prefs.calendar_enabled != true THEN
        RETURN NEW;
    END IF;
    
    SELECT * INTO scholarship_info FROM public.scholarships WHERE id = NEW.scholarship_id;
    ...
    INSERT INTO public.calendar_events (
        user_id, scholarship_id, calendar_provider, external_event_id,
        title, description, start_time, end_time, sync_status
    ) VALUES (
        NEW.user_id, NEW.scholarship_id,
        ...
        'Scholarship application deadline for ' || scholarship_info.title,
        scholarship_info.deadline - INTERVAL '2 hours',
        scholarship_info.deadline, 'pending'
...
CREATE FUNCTION get_users_needing_reminders()
RETURNS TABLE (
    user_id UUID, user_email TEXT, scholarship_id UUID,
    scholarship_title TEXT, provider TEXT, deadline TIMESTAMPTZ,
    days_until_deadline INTEGER, urgency TEXT, 
    reminder_days_before INTEGER, email_preferences JSONB
)
LANGUAGE plpgsql
SET search_path = ''
AS $$
BEGIN
    RETURN QUERY
    SELECT 
        usi.user_id, au.email as user_email, s.id as scholarship_id,
        s.title as scholarship_title, s.provider, s.deadline,
    ...
    WHERE s.deadline >= NOW() AND s.deadline <= NOW() + INTERVAL '31 days'
...
CREATE FUNCTION get_application_dashboard(user_uuid UUID)
RETURNS TABLE (
    sop_id BIGINT, university_name TEXT, program_name TEXT,
    country TEXT, program_type TEXT, application_deadline DATE,
    status TEXT, word_count INTEGER, created_at TIMESTAMP WITH TIME ZONE,
    updated_at TIMESTAMP WITH TIME ZONE, days_until_deadline INTEGER,
...
BEGIN
    RETURN QUERY
    SELECT 
        s.id::BIGINT as sop_id, s.university_name, s.program_name,
        s.country, s.program_type, s.application_deadline,
        s.status, s.word_count, s.created_at, s.updated_at,
        CASE 
    ...
    ORDER BY 
        ...
        s.application_deadline ASC, s.updated_at DESC;
END;
$$;

CREATE FUNCTION get_user_sop_stats(user_uuid UUID)
RETURNS TABLE (
    total_sops INTEGER, draft_sops INTEGER, final_sops INTEGER,
    submitted_applications INTEGER, pending_deadlines INTEGER,
    overdue_deadlines INTEGER, countries_applied INTEGER,
    avg_word_count INTEGER, most_recent_activity TIMESTAMP WITH TIME ZONE
)
LANGUAGE plpgsql
SET search_path = ''
AS $$
...
CREATE FUNCTION increment_usage(user_uuid UUID, usage_type TEXT, increment_by INTEGER DEFAULT 1)
RETURNS BOOLEAN
LANGUAGE plpgsql
SET search_path = ''
AS $$
