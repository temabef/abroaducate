...
BEGIN
    ...
    FOREACH func_record.proname IN ARRAY func_names
    LOOP
        BEGIN
            -- Attempt to alter the function to set a secure search_path
            EXECUTE format('ALTER FUNCTION public.%s SET search_path FROM CURRENT', func_record.proname);
            RAISE NOTICE '✅ Fixed search_path for function: %', func_record.proname;
        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE '⚠️ Could not fix function: % - Error: %', func_record.proname, SQLERRM;
        END;
    END LOOP;
