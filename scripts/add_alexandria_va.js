...
const data = {
  ...
  population: 159000,
  popular_universities: ['Northern Virginia Community College', 'George Washington University (nearby)', 'American University (nearby)']
};

async function run(){
  try{
    console.log('Adding Alexandria, Virginia...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Alexandria added/updated.');
  }catch(e){
    console.error('❌ Failed to add Alexandria:', e.message);
    process.exit(1);
  }
}
