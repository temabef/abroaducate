// Add Knoxville, TN (University of Tennessee)
...
const data = {
  ...
  population: 190000,
  popular_universities: ['University of Tennessee', 'Pellissippi State Community College', 'Johnson University']
};

async function run(){
  try{
    console.log('Adding Knoxville, Tennessee...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Knoxville added/updated.');
  }catch(e){
    console.error('❌ Failed to add Knoxville:', e.message);
    process.exit(1);
  }
}
