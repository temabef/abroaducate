// Add Baltimore, MD (Johns Hopkins University)
...
const data = {
  ...
  population: 585000,
  popular_universities: ['Johns Hopkins University', 'University of Maryland Baltimore', 'Loyola University Maryland', 'Morgan State University', 'Baltimore City Community College']
};

async function run(){
  try{
    console.log('Adding Baltimore, Maryland...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Baltimore added/updated.');
  }catch(e){
    console.error('❌ Failed to add Baltimore:', e.message);
    process.exit(1);
  }
}
