...
const data = {
  ...
  population: 100000,
  popular_universities: ['Central Oregon Community College', 'Oregon State University-Cascades']
};

async function run(){
  try{
    console.log('Adding Bend, Oregon...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Bend added/updated.');
  }catch(e){
    console.error('❌ Failed to add Bend:', e.message);
    process.exit(1);
  }
}
