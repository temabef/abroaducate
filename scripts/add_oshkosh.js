// Add Oshkosh, WI (University of Wisconsin-Oshkosh)
...
const data = {
  ...
  population: 67000,
  popular_universities: ['University of Wisconsin-Oshkosh', 'Fox Valley Technical College', 'Marian University']
};

async function run(){
  try{
    console.log('Adding Oshkosh, Wisconsin...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Oshkosh added/updated.');
  }catch(e){
    console.error('❌ Failed to add Oshkosh:', e.message);
    process.exit(1);
  }
}
