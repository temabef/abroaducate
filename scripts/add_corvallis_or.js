// Add Corvallis, OR (Oregon State University)
...
const data = {
  ...
  population: 58000,
  popular_universities: ['Oregon State University', 'Linn-Benton Community College']
};

async function run(){
  try{
    console.log('Adding Corvallis, Oregon...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Corvallis added/updated.');
  }catch(e){
    console.error('❌ Failed to add Corvallis:', e.message);
    process.exit(1);
  }
}
