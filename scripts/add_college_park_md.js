// Add College Park, MD (University of Maryland)
...
const data = {
  ...
  population: 35000,
  popular_universities: ['University of Maryland', 'University of Maryland Global Campus', 'Prince George\'s Community College']
};

async function run(){
  try{
    console.log('Adding College Park, Maryland...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ College Park added/updated.');
  }catch(e){
    console.error('❌ Failed to add College Park:', e.message);
    process.exit(1);
  }
}
