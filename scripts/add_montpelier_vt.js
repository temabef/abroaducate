...
const data = {
  ...
  population: 8000,
  popular_universities: ['Vermont College of Fine Arts', 'Community College of Vermont']
};

async function run(){
  try{
    console.log('Adding Montpelier, Vermont...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Montpelier added/updated.');
  }catch(e){
    console.error('❌ Failed to add Montpelier:', e.message);
    process.exit(1);
  }
}
