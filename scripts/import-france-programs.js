/**
 * Import France programs from the enriched platform CSV into Supabase.
 *
 * Usage:
 *   node scripts/import-france-programs.js --dry-run
 *   node scripts/import-france-programs.js --apply
 *   node scripts/import-france-programs.js --apply --replace-country
 *   node scripts/import-france-programs.js --dry-run --csv path/to/file.csv
 *
 * This script mirrors import-lithuania-programs.js (same target schema and
 * application-platform conventions), with:
 *   - country label set to "France"
 *   - living cost default EUR 900 / month
 *   - default CSV is france_final_upload_enriched.csv (post cleanup + deadline
 *     enrichment)
 */
...
const DEFAULT_CSV_CANDIDATES = [
  resolve(__dirname, '..', 'france_final_upload_enriched.csv'),
  resolve(__dirname, '..', 'france_final_upload_cleaned.csv'),
  resolve(__dirname, '..', 'france_final_upload.csv')
];
...
if (!CSV_PATH || !existsSync(CSV_PATH)) {
  console.error('Could not find a France CSV. Pass one with --csv path/to/file.csv');
  process.exit(1);
}
...
function buildProgramId(row, rowIndex) {
  ...
  const base = slugify(
    [row.country || 'france', row.university_name, row.program_name, row.city, row.degree_level]
      .filter(Boolean)
      .join('-')
  ).substring(0, 100);
  ...
  return {
    ...
    funding_pathway_explanation:
      tuitionPerSemester > 0
        ? 'This programme has tuition for international students. Plan admission alongside scholarship research (Eiffel, BGF, Mastercard Foundation) and personal funding preparation.'
        : 'This programme appears tuition-free in the source data. Focus funding planning on living costs, CVEC, and French long-stay visa requirements after admission.',
    ...
    notes: 'Imported from France programme CSV (enriched with researched deadlines).',
    is_active: true,
    ...
    for (const [key, uni] of universityMap) {
      ...
      const { data: inserted, error: insertError } = await supabase
        ...
        .insert({
          ...
          description: `${uni.name} is a French institution offering programmes for international students.`,
