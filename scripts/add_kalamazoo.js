// Add Kalamazoo, MI (Western Michigan University)
...
const data = {
  ...
  population: 76000,
  popular_universities: ['Western Michigan University', 'Kalamazoo College', 'Kalamazoo Valley Community College']
};

async function run(){
  try{
    console.log('Adding Kalamazoo, Michigan...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Kalamazoo added/updated.');
  }catch(e){
    console.error('❌ Failed to add Kalamazoo:', e.message);
    process.exit(1);
  }
}
