// Add Des Moines, IA (Drake University)
...
const data = {
  ...
  population: 214000,
  popular_universities: ['Drake University', 'Grand View University', 'Des Moines Area Community College']
};

async function run(){
  try{
    console.log('Adding Des Moines, Iowa...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Des Moines added/updated.');
  }catch(e){
    console.error('❌ Failed to add Des Moines:', e.message);
    process.exit(1);
  }
}
