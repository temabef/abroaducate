...
const data = {
  ...
  population: 44000,
  popular_universities: ['New Hampshire Technical Institute', 'Granite State College', 'NHTI-Concord\'s Community College']
};

async function run(){
  try{
    console.log('Adding Concord, New Hampshire...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Concord added/updated.');
  }catch(e){
    console.error('❌ Failed to add Concord:', e.message);
    process.exit(1);
  }
}
