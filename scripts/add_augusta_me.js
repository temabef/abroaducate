...
const data = {
  ...
  population: 19000,
  popular_universities: ['University of Maine at Augusta', 'Kennebec Valley Community College']
};

async function run(){
  try{
    console.log('Adding Augusta, Maine...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Augusta added/updated.');
  }catch(e){
    console.error('❌ Failed to add Augusta:', e.message);
    process.exit(1);
  }
}
