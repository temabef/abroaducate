// Add San Antonio, TX (University of Texas at San Antonio)
...
const data = {
  ...
  population: 1540000,
  popular_universities: ['University of Texas at San Antonio', 'Trinity University', 'St. Mary\'s University', 'San Antonio College']
};

async function run(){
  try{
    console.log('Adding San Antonio, Texas...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ San Antonio added/updated.');
  }catch(e){
    console.error('❌ Failed to add San Antonio:', e.message);
    process.exit(1);
  }
}
