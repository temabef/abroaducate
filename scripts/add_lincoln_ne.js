// Add Lincoln, NE (University of Nebraska Lincoln)
...
const data = {
  ...
  population: 289000,
  popular_universities: ['University of Nebraska Lincoln', 'Nebraska Wesleyan University', 'Southeast Community College']
};

async function run(){
  try{
    console.log('Adding Lincoln, Nebraska...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Lincoln (NE) added/updated.');
  }catch(e){
    console.error('❌ Failed to add Lincoln (NE):', e.message);
    process.exit(1);
  }
}
