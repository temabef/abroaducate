// Add Huntsville, AL (University of Alabama in Huntsville)
...
const data = {
  ...
  population: 215000,
  popular_universities: ['University of Alabama in Huntsville', 'Alabama A&M University', 'Oakwood University']
};

async function run(){
  try{
    console.log('Adding Huntsville, Alabama...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Huntsville added/updated.');
  }catch(e){
    console.error('❌ Failed to add Huntsville:', e.message);
    process.exit(1);
  }
}
