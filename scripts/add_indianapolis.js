// Add Indianapolis, IN (Indiana University-Purdue University Indianapolis)
...
const data = {
  ...
  population: 880000,
  popular_universities: ['Indiana University-Purdue University Indianapolis (IUPUI)', 'Butler University', 'Marian University', 'University of Indianapolis']
};

async function run(){
  try{
    console.log('Adding Indianapolis, Indiana...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Indianapolis added/updated.');
  }catch(e){
    console.error('❌ Failed to add Indianapolis:', e.message);
    process.exit(1);
  }
}
