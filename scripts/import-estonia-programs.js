/**
 * Import Estonia programs from the cleaned platform CSV into Supabase.
 *
 * Usage:
 *   node scripts/import-estonia-programs.js --dry-run
 *   node scripts/import-estonia-programs.js --apply
 *   node scripts/import-estonia-programs.js --apply --replace-country
 *   node scripts/import-estonia-programs.js --dry-run --csv path/to/file.csv
 */
...
const DEFAULT_CSV_CANDIDATES = [
  resolve(__dirname, '..', 'estonia_final_upload.csv'),
  resolve(__dirname, '..', 'estonia_final_upload_cleaned.csv')
];
...
if (!CSV_PATH || !existsSync(CSV_PATH)) {
  console.error('Could not find an Estonia CSV. Pass one with --csv path/to/file.csv');
  process.exit(1);
}
...
function buildApplicationSteps(row) {
  ...
  return [
    ...
    },
    {
      ...
      description: 'After admission, finalize tuition planning, accommodation, and residence documentation for Estonia.',
      type: 'visa_prep',
...
function mapRow(row, universityIdMap, rowIndex) {
  ...
  const livingCost = toNumber(row.living_cost_per_month, 400);
  ...
  return {
    ...
    notes: 'Imported from Estonia programme CSV.',
    is_active: true,
...
async function main() {
  ...
  console.log(`Replace existing Estonia programs: ${REPLACE_COUNTRY ? 'yes' : 'no'}`);
    ...
    for (const [key, uni] of universityMap) {
      ...
      const { data: inserted, error: insertError } = await supabase
        ...
        .insert({
          ...
          description: `${uni.name} is an Estonian institution offering programmes for international students.`,
          living_cost_estimate: 'EUR 400 / month'
        })
        .select('id')
        .single();
  ...
  if (REPLACE_COUNTRY) {
    ...
    console.log(`Deleted ${existingCount || 0} existing Estonia program rows before replacement.`);
  }
  ...
  console.log(`Import complete. Upserted ${inserted} Estonia programs.`);
}
