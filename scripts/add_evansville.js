// Add Evansville, IN (University of Evansville)
...
const data = {
  ...
  population: 118000,
  popular_universities: ['University of Evansville', 'University of Southern Indiana', 'Ivy Tech Community College']
};

async function run(){
  try{
    console.log('Adding Evansville, Indiana...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Evansville added/updated.');
  }catch(e){
    console.error('❌ Failed to add Evansville:', e.message);
    process.exit(1);
  }
}
