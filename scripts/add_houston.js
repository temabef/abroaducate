// Add Houston, TX (University of Houston)
...
const data = {
  ...
  population: 2320000,
  popular_universities: ['University of Houston', 'Rice University', 'Texas Southern University', 'Houston Community College']
};

async function run(){
  try{
    console.log('Adding Houston, Texas...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Houston added/updated.');
  }catch(e){
    console.error('❌ Failed to add Houston:', e.message);
    process.exit(1);
  }
}
