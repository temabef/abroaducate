// Add Akron, OH (University of Akron)
...
const data = {
  ...
  population: 197000,
  popular_universities: ['University of Akron', 'Kent State University', 'Stark State College']
};

async function run(){
  try{
    console.log('Adding Akron, Ohio...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Akron added/updated.');
  }catch(e){
    console.error('❌ Failed to add Akron:', e.message);
    process.exit(1);
  }
}
