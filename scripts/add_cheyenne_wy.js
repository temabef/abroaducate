...
const data = {
  ...
  population: 65000,
  popular_universities: ['Laramie County Community College', 'University of Wyoming (nearby in Laramie)']
};

async function run(){
  try{
    console.log('Adding Cheyenne, Wyoming...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Cheyenne added/updated.');
  }catch(e){
    console.error('❌ Failed to add Cheyenne:', e.message);
    process.exit(1);
  }
}
