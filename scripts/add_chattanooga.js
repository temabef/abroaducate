// Add Chattanooga, TN (University of Tennessee at Chattanooga)
...
const data = {
  ...
  population: 181000,
  popular_universities: ['University of Tennessee at Chattanooga', 'Chattanooga State Community College', 'Southern Adventist University']
};

async function run(){
  try{
    console.log('Adding Chattanooga, Tennessee...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Chattanooga added/updated.');
  }catch(e){
    console.error('❌ Failed to add Chattanooga:', e.message);
    process.exit(1);
  }
}
