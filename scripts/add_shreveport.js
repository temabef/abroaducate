// Add Shreveport, LA (Louisiana State University Shreveport)
...
const data = {
  ...
  population: 187000,
  popular_universities: ['Louisiana State University Shreveport', 'Centenary College of Louisiana', 'Southern University Shreveport']
};

async function run(){
  try{
    console.log('Adding Shreveport, Louisiana...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Shreveport added/updated.');
  }catch(e){
    console.error('❌ Failed to add Shreveport:', e.message);
    process.exit(1);
  }
}
