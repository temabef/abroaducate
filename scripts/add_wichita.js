// Add Wichita, KS (Wichita State University)
...
const data = {
  ...
  population: 397000,
  popular_universities: ['Wichita State University', 'Friends University', 'Wichita Area Technical College']
};

async function run(){
  try{
    console.log('Adding Wichita, Kansas...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Wichita added/updated.');
  }catch(e){
    console.error('❌ Failed to add Wichita:', e.message);
    process.exit(1);
  }
}
