// Add Grand Rapids, MI (Grand Valley State University)
...
const data = {
  ...
  population: 200000,
  popular_universities: ['Grand Valley State University', 'Calvin University', 'Davenport University', 'Grand Rapids Community College']
};

async function run(){
  try{
    console.log('Adding Grand Rapids, Michigan...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Grand Rapids added/updated.');
  }catch(e){
    console.error('❌ Failed to add Grand Rapids:', e.message);
    process.exit(1);
  }
}
