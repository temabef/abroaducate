// Add Omaha, NE (University of Nebraska Omaha)
...
const data = {
  ...
  population: 486000,
  popular_universities: ['University of Nebraska Omaha', 'Creighton University', 'Metropolitan Community College']
};

async function run(){
  try{
    console.log('Adding Omaha, Nebraska...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Omaha added/updated.');
  }catch(e){
    console.error('❌ Failed to add Omaha:', e.message);
    process.exit(1);
  }
}
