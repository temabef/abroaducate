// Add Toledo, OH (University of Toledo)
...
const data = {
  ...
  population: 275000,
  popular_universities: ['University of Toledo', 'Bowling Green State University', 'Owens Community College']
};

async function run(){
  try{
    console.log('Adding Toledo, Ohio...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Toledo added/updated.');
  }catch(e){
    console.error('❌ Failed to add Toledo:', e.message);
    process.exit(1);
  }
}
