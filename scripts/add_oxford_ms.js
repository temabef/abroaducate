// Add Oxford, MS (University of Mississippi)
...
const data = {
  ...
  population: 28000,
  popular_universities: ['University of Mississippi (Ole Miss)', 'Northwest Mississippi Community College']
};

async function run(){
  try{
    console.log('Adding Oxford, Mississippi...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Oxford (MS) added/updated.');
  }catch(e){
    console.error('❌ Failed to add Oxford (MS):', e.message);
    process.exit(1);
  }
}
