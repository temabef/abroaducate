...
const data = {
  ...
  // City info
  cost_of_living_index: 115.3,
  population: 190000,
  popular_universities: ['Washington State University Vancouver', 'Clark College', 'Portland State University (nearby in Oregon)']
};

async function run(){
  try{
    console.log('Adding Vancouver, Washington...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Vancouver (WA) added/updated.');
  }catch(e){
    console.error('❌ Failed to add Vancouver (WA):', e.message);
    process.exit(1);
  }
}
