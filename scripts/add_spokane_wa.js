...
const data = {
  ...
  // City info
  cost_of_living_index: 105.8,
  population: 220000,
  popular_universities: ['Gonzaga University', 'Whitworth University', 'Eastern Washington University', 'Spokane Community College', 'Spokane Falls Community College']
};

async function run(){
  try{
    console.log('Adding Spokane, Washington...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Spokane added/updated.');
  }catch(e){
    console.error('❌ Failed to add Spokane:', e.message);
    process.exit(1);
  }
}
