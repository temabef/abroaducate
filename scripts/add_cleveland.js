// Add Cleveland, OH (Case Western Reserve University)
...
const data = {
  ...
  // City info
  cost_of_living_index: 94.8,
  population: 372000,
  popular_universities: ['Case Western Reserve University', 'Cleveland State University', 'John Carroll University', 'Cuyahoga Community College']
};

async function run(){
  try{
    console.log('Adding Cleveland, Ohio...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Cleveland added/updated.');
  }catch(e){
    console.error('❌ Failed to add Cleveland:', e.message);
    process.exit(1);
  }
}
