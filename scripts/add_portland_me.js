...
const data = {
  ...
  // City info
  cost_of_living_index: 110.2,
  population: 68000,
  popular_universities: ['University of Southern Maine', 'Maine College of Art', 'Southern Maine Community College', 'University of New England']
};

async function run(){
  try{
    console.log('Adding Portland, Maine...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Portland added/updated.');
  }catch(e){
    console.error('❌ Failed to add Portland:', e.message);
    process.exit(1);
  }
}
