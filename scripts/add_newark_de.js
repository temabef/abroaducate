// Add Newark, DE (University of Delaware)
...
const data = {
  ...
  // City info
  cost_of_living_index: 112.4,
  population: 32000,
  popular_universities: ['University of Delaware', 'Delaware Technical Community College']
};

async function run(){
  try{
    console.log('Adding Newark, Delaware...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Newark (DE) added/updated.');
  }catch(e){
    console.error('❌ Failed to add Newark (DE):', e.message);
    process.exit(1);
  }
}
