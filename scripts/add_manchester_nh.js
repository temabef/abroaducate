...
const data = {
  ...
  population: 112000,
  popular_universities: ['University of New Hampshire at Manchester', 'Southern New Hampshire University', 'Manchester Community College', 'New Hampshire Institute of Art']
};

async function run(){
  try{
    console.log('Adding Manchester, New Hampshire...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Manchester added/updated.');
  }catch(e){
    console.error('❌ Failed to add Manchester:', e.message);
    process.exit(1);
  }
}
