// Add Mobile, AL (University of South Alabama)
...
const data = {
  ...
  population: 187000,
  popular_universities: ['University of South Alabama', 'Spring Hill College', 'Bishop State Community College']
};

async function run(){
  try{
    console.log('Adding Mobile, Alabama...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Mobile added/updated.');
  }catch(e){
    console.error('❌ Failed to add Mobile:', e.message);
    process.exit(1);
  }
}
