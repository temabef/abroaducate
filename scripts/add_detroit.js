// Add Detroit, MI (Wayne State University)
...
const data = {
  ...
  population: 674000,
  popular_universities: ['Wayne State University', 'University of Detroit Mercy', 'Marygrove College', 'Henry Ford College']
};

async function run(){
  try{
    console.log('Adding Detroit, Michigan...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Detroit added/updated.');
  }catch(e){
    console.error('❌ Failed to add Detroit:', e.message);
    process.exit(1);
  }
}
