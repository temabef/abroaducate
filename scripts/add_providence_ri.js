// Add Providence, RI (Brown University)
...
const data = {
  ...
  // City info
  cost_of_living_index: 119.2,
  population: 190000,
  popular_universities: ['Brown University', 'Rhode Island School of Design', 'Providence College', 'Johnson & Wales University', 'Rhode Island College', 'Community College of Rhode Island']
};

async function run(){
  try{
    console.log('Adding Providence, Rhode Island...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Providence added/updated.');
  }catch(e){
    console.error('❌ Failed to add Providence:', e.message);
    process.exit(1);
  }
}
