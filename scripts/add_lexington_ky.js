// Add Lexington, KY (University of Kentucky)
...
const data = {
  ...
  population: 320000,
  popular_universities: ['University of Kentucky', 'Transylvania University', 'Bluegrass Community and Technical College']
};

async function run(){
  try{
    console.log('Adding Lexington, Kentucky...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Lexington (KY) added/updated.');
  }catch(e){
    console.error('❌ Failed to add Lexington (KY):', e.message);
    process.exit(1);
  }
}
