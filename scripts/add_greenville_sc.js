// Add Greenville, SC (Clemson University)
...
const data = {
  ...
  population: 72000,
  popular_universities: ['Clemson University', 'Furman University', 'Greenville Technical College']
};

async function run(){
  try{
    console.log('Adding Greenville, South Carolina...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Greenville (SC) added/updated.');
  }catch(e){
    console.error('❌ Failed to add Greenville (SC):', e.message);
    process.exit(1);
  }
}
