// Add Youngstown, OH (Youngstown State University)
...
const data = {
  ...
  population: 65000,
  popular_universities: ['Youngstown State University', 'Kent State University at Trumbull', 'Eastern Gateway Community College']
};

async function run(){
  try{
    console.log('Adding Youngstown, Ohio...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Youngstown added/updated.');
  }catch(e){
    console.error('❌ Failed to add Youngstown:', e.message);
    process.exit(1);
  }
}
