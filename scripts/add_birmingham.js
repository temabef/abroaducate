// Add Birmingham, AL (University of Alabama at Birmingham)
...
const data = {
  ...
  population: 200000,
  popular_universities: ['University of Alabama at Birmingham', 'Samford University', 'Birmingham-Southern College']
};

async function run(){
  try{
    console.log('Adding Birmingham, Alabama...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Birmingham added/updated.');
  }catch(e){
    console.error('❌ Failed to add Birmingham:', e.message);
    process.exit(1);
  }
}
