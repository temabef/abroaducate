// Add Lansing, MI (Michigan State University)
...
const data = {
  ...
  population: 118000,
  popular_universities: ['Michigan State University', 'Lansing Community College', 'Davenport University']
};

async function run(){
  try{
    console.log('Adding Lansing, Michigan...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Lansing added/updated.');
  }catch(e){
    console.error('❌ Failed to add Lansing:', e.message);
    process.exit(1);
  }
}
