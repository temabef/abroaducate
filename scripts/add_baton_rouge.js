// Add Baton Rouge, LA (Louisiana State University)
...
const data = {
  ...
  population: 221000,
  popular_universities: ['Louisiana State University', 'Southern University', 'Baton Rouge Community College']
};

async function run(){
  try{
    console.log('Adding Baton Rouge, Louisiana...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Baton Rouge added/updated.');
  }catch(e){
    console.error('❌ Failed to add Baton Rouge:', e.message);
    process.exit(1);
  }
}
