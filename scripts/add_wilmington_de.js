...
const data = {
  ...
  // City info
  cost_of_living_index: 108.9,
  population: 72000,
  popular_universities: ['University of Delaware', 'Delaware State University', 'Delaware Technical Community College', 'Widener University Delaware Law School']
};

async function run(){
  try{
    console.log('Adding Wilmington, Delaware...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Wilmington added/updated.');
  }catch(e){
    console.error('❌ Failed to add Wilmington:', e.message);
    process.exit(1);
  }
}
