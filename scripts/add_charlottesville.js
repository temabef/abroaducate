// Add Charlottesville, VA (University of Virginia)
...
const data = {
  ...
  // City info
  cost_of_living_index: 114.7,
  population: 47000,
  popular_universities: ['University of Virginia', 'Piedmont Virginia Community College']
};

async function run(){
  try{
    console.log('Adding Charlottesville, Virginia...');
    const { error } = await supabase.from('us_cities_cost_data').upsert([data]);
    if (error) throw error;
    console.log('✅ Charlottesville added/updated.');
  }catch(e){
    console.error('❌ Failed to add Charlottesville:', e.message);
    process.exit(1);
  }
}
