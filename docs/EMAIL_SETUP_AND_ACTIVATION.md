...
| **Registered (paid)** | Application deadline reminders, subscription/trial alerts | As deadlines approach / 7â€“3â€“1 days before expiry | Same cron |
...
**Where the email is designed:** In code only â€” `src/routes/api/cron/send-reminders/+server.ts`: `generateScholarshipDigestHTML()` and `generateScholarshipDigestText()`. Template is inline HTML (title, provider, amount, deadline, link to `/scholarships/{id}`). To change design: edit that file, or later add admin-editable templates (e.g. in DB or CMS).
...
| Piece | Status | Where |
|-------|--------|--------|
| **SendGrid dependency** | âœ… In use | `@sendgrid/mail` in `package.json` |
| **Newsletter DB** | âœ… Migrated | `newsletter_subscribers`, `newsletter_campaigns`, `newsletter_email_logs`, `newsletter_settings` (Supabase) |
| **Admin UI** | âœ… Working | `/admin/newsletter` â€“ compose, batch send, test SendGrid, analytics, subscriber list |
| **Batch send API** | âœ… Working | `POST /api/newsletter/batch-send` â€“ sends to active `newsletter_subscribers` in batches |
| **Test SendGrid** | âœ… Working | `GET /api/newsletter/test-sendgrid` â€“ sends one test email to logged-in admin |
| **Contact support** | âœ… Working | `POST /api/contact-support` â€“ uses SendGrid to send to you + auto-reply |
| **Cron reminders** | âœ… Implemented | `POST /api/cron/send-reminders` â€“ scholarship digest, application reminders, trial/subscription alerts for **registered users** (needs `CRON_SECRET` + `SENDGRID_API_KEY`) |
| **Email reminders (user-triggered)** | âœ… Working | `POST /api/email-reminders` â€“ per-user reminder emails (respects preferences) |
...
|----------|----------|--------|--------|
| **SENDGRID_API_KEY** | âœ… Yes (for sending) | Newsletter, contact, cron, test-email | Create in SendGrid Dashboard â†’ API Keys |
| **FROM_EMAIL** | âœ… Yes | All SendGrid sends | e.g. `hello@abroaducate.com` â€“ **must be verified in SendGrid** (single sender or domain) |
...
| **CRON_SECRET** | âœ… Yes (for cron) | `/api/cron/send-reminders` | Secret token; caller must send `Authorization: Bearer <CRON_SECRET>` |

- **Vercel:** set these in Project â†’ Settings â†’ Environment Variables (Production, Preview if you want).
- **Local:** add to `.env` (do not commit `.env`).

### 1.3 Why emails are â€œnot workingâ€

- **SendGrid API key** is missing or free tier only â†’ no sending, or 100/day limit.
- **FROM_EMAIL** not set or not verified in SendGrid â†’ sends fail or get blocked.
- **Cron** for reminders is already scheduled: a **GitHub Actions workflow** (`.github/workflows/cron-reminders.yml`) runs daily at 9:00 UTC and calls `POST /api/cron/send-reminders` with `Authorization: Bearer <CRON_SECRET>`. You must set **CRON_SECRET** in the repoâ€™s GitHub Actions secrets so the workflow can authenticate.
...
- **Abroaducate (you)** send:
  ...
  - **Digest and reminders:** via `/api/cron/send-reminders` for registered users (scholarship digest, application deadlines, â€œyour trial ends in X daysâ€), and any one-off reminder flows that call `email-reminders` or similar.
...
- **Free:** 100 emails/day â†’ not enough for 12k people.
- **Essentials (e.g. ~$19.95/mo):** Often 50,000 emails/month. Enough for:
  - ~12k people Ã— 1â€“4 emails/month = 12kâ€“48k/month (e.g. weekly digest + 1â€“2 campaigns), or
  - Fewer people with more frequent emails.
- **Pro and above:** Higher volume if you grow.

**Recommendation:** Start with the **paid plan youâ€™re considering (e.g. Essentials)**. Before paying: (1) verify sender (see below), (2) set env vars, (3) run the test send. After paying, youâ€™re just under the same config; no code change needed.
...
## 5. What to do before you pay (setup so itâ€™s ready)

### Step 1: SendGrid account and sender

1. Sign up / log in at [SendGrid](https://sendgrid.com).
2. Create an **API Key** (Dashboard â†’ API Keys) with â€œMail Sendâ€ (and optionally â€œMail Send â€“ Scheduledâ€ if you use scheduling). Copy the key once; youâ€™ll put it in `SENDGRID_API_KEY`.
...
3. Click **"Test SendGrid Configuration"** (or call `GET /api/newsletter/test-sendgrid`).  
   - If `SENDGRID_API_KEY` is missing â†’ youâ€™ll get a clear error.  
   - If the key is set but the sender is not verified â†’ SendGrid will reject the test send; fix verification then retry.  
   - If the test succeeds â†’ youâ€™re ready; after you pay, the same key will work for full volume.

### Step 4: After you pay

- No code or env changes needed.
- Send a small batch from `/admin/newsletter` (e.g. â€œSend to first 50â€) to confirm.
- Then decide frequency and content (Section 6).

---

## 6. Frequency and types of emails (donâ€™t bombard)

Recommended approach: **one main rhythm + a few clear types**, so people know what to expect and donâ€™t unsubscribe.

### 6.1 Newsletter list (~8k)

- **Weekly digest (e.g. Mondays):** One â€œtop scholarships / study abroad tipsâ€ email per week.  
  - Implement by: a cron (or Vercel Cron) that builds a campaign and calls your batch-send or a dedicated â€œweekly digestâ€ endpoint, or you send manually from `/admin/newsletter` once a week.
- **Occasional campaigns:** 1â€“2 extra emails per month (e.g. â€œNew featureâ€, â€œApplication deadline reminderâ€), not daily.
- **Avoid:** Daily blasts to the whole list; that will hurt deliverability and unsubscribes.

So: **max ~4â€“6 emails per month per subscriber** for the newsletter list is a safe target.
...
- **Application/deadline reminders:** From cron; keep to a few per week per user (e.g. â€œ3 deadlines in the next 7 daysâ€).
- **Subscription/trial alerts:** From cron (e.g. â€œTrial ends in 3 daysâ€); these are transactional and expected.
- **Marketing to registered users:** If you add a â€œnewsletterâ€ segment for users, treat like the 8k list: weekly or a few per month, not daily.
...
|----------|--------------------------|--------|
| Newsletter (8k) | Weekly digest + 1â€“2 campaigns/month | Digest, study tips, soft promo |
...
Sending is **automatic only** (no one-time manual script). The daily cron (GitHub Actions) runs the endpoint; digest and reminder logic (e.g. weekly on Monday for free users, daily for paid) is handled inside the handler.
...
- [ ] Decide: weekly (or daily for paid) digest + 1â€“2 campaigns/month for the 8k list; stick to that so you donâ€™t bombard.
...
**Summary:** The email system is implemented and wired to SendGrid and your DB. A **GitHub Actions workflow** already runs the reminder/digest cron daily at 9:00 UTC. Set `SENDGRID_API_KEY` and `FROM_EMAIL` (verified in SendGrid), set `CRON_SECRET` in both Vercel and GitHub Actions secrets, run the test send, then pay for SendGrid. After paying, the same config works and the existing cron will send digest/reminders to registered users; keep newsletter frequency to weekly (+ 1â€“2 campaigns/month) for the 8k list so you donâ€™t bombard anyone.
