-- ========================================
-- FIX USER MANAGEMENT ANALYTICS
-- Simple working function for user statistics
-- ========================================
...
-- Create a simple, reliable analytics function
...
BEGIN
    ...
    SELECT 
        COUNT(*) FILTER (WHERE plan_type = 'professional' AND status = 'active'),
        COUNT(*) FILTER (WHERE plan_type = 'elite' AND status = 'active')
    INTO professional_count, elite_count
    FROM user_subscriptions;
    
    -- Calculate free users
    free_count := GREATEST(0, total_users_count - professional_count - elite_count);
...
-- Test the function
SELECT 'Testing get_dashboard_analytics function...';
