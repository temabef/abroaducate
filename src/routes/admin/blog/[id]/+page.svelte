<script lang="ts">
  ...
  let cover_image_url = $state(post.cover_image_url || '');
  ...
  function onTitleChange(e: Event) {
    ...
    title = target.value;
    // Don't auto-update slug for existing posts to avoid breaking links
  }
...
<div class="container mx-auto px-4 py-8 max-w-4xl">
  ...
  <form method="POST" class="space-y-6">
    ...
    <div>
      ...
      />
      <p class="text-sm text-gray-500 mt-1">Be careful changing this as it will break existing links</p>
    </div>
      ...
      <div class="flex gap-3">
        ...
        {#if post.status === 'published'}
          <a
            href="/blog/{post.slug}"
            target="_blank"
            class="px-6 py-2 border border-green-300 text-green-700 rounded-lg hover:bg-green-50 transition-colors"
          >
            View Live
          </a>
        {/if}
