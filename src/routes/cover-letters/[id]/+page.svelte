<script lang="ts">
	...
	import { ChevronLeft, Save, Copy, Check, Mail, Download } from 'lucide-svelte';

	let { data } = $props();
	let supabase = $derived(data.supabase);
	let coverLetter = $state<any>(data.coverLetter);

	let content = $state<string>(coverLetter?.content ?? coverLetter?.generated_content ?? '');
	let title = $state<string>(
		coverLetter?.job_title
			? `${coverLetter.job_title}${coverLetter.company_name ? ' — ' + coverLetter.company_name : ''}`
			: coverLetter?.program_name ?? 'Cover Letter'
	);
	...
	async function save() {
		if (!supabase || !coverLetter) return;
		saving = true;
	...
	async function exportWord() {
		...
		const res = await fetch('/api/export-word', {
			...
			body: JSON.stringify({ content, title, type: 'cover-letter' })
		});
		if (res.ok) {
...
<svelte:head>
	<title>{title} | Cover Letter | Abroaducate</title>
</svelte:head>
	...
	<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		<div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
			<div class="border-b border-slate-100 px-6 py-3 flex items-center justify-between">
				<span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Cover Letter</span>
			...
			<textarea
				...
				placeholder="Start writing your cover letter here, or paste your existing draft..."
			></textarea>
		</div>
