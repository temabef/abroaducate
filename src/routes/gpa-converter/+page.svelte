<script lang="ts">
	...
	import {
		africanCountries,
		africanGradingSystems,
		getAfricanCountryGradingSystems
	...
	// Reactive variables
	let availableGradingSystems = $derived(
		selectedCountry ? Object.keys(getAfricanCountryGradingSystems(selectedCountry)) : []
	);
	let currentGradingSystem = $derived(
		selectedCountry && selectedGradingSystem
			? (getAfricanCountryGradingSystems(selectedCountry) as any)[selectedGradingSystem]
			: null
	);
	...
	async function downloadTranscript() {
		...
		try {
			...
			const coursesByYear = academicYears.reduce(
				(acc, year) => {
			...
			Object.entries(coursesByYear).forEach(([year, yearCourses]) => {
				...
				yearCourses.forEach((course, index) => {
					...
					const courseName =
						course.name.length > 35 ? course.name.substring(0, 35) + '...' : course.name;
					...
					// Color-code GPA
					const gpaColor =
						course.usGPA >= 3.5 ? [0, 128, 0] : course.usGPA >= 2.5 ? [255, 165, 0] : [220, 20, 60];
			...
			doc.text(
				'This document represents an automated conversion of academic grades based on the specified grading system.',
				105,
				footerY + 10,
				{ align: 'center' }
			);
			doc.text('For academic application and transcript evaluation purposes.', 105, footerY + 15, {
				align: 'center'
			});
			doc.text(`Document prepared: ${new Date().toLocaleString()}`, 105, footerY + 20, {
				align: 'center'
			});
			...
			alert(
				`Failed to generate PDF: ${error?.message || error}. Please check console for details.`
			);
		}
	}
	...
	function handleAnalyzeAcademicProfile() {
		...
		if (courses.length < 3) {
			const proceed = confirm(
				`You only have ${courses.length} course(s) added. For meaningful analysis, we recommend at least 5 courses. Would you like to continue anyway?`
			);
			if (!proceed) return;
		}
		...
		try {
			...
			import('$lib/academicAnalyzer/analysisEngine.js')
				.then((module) => {
					...
					if (!analysis || !analysis.hasAnalysis) {
						alert(
							'Unable to generate meaningful analysis with current data. Please add more courses or check your entries.'
						);
						return;
					}
					...
					if (!hasMinimumInsights) {
						const proceed = confirm(
							'Analysis completed, but we found limited insights with your current data. This may result in sparse results. Continue viewing?'
						);
						if (!proceed) return;
					}
		...
		academicYears.forEach((year) => {
			...
			if (yearCourses.length > 0) {
				...
				const yearPoints = yearCourses.reduce(
					(sum, course) => sum + course.usGPA * course.credits,
					0
				);
				coursesByYear[year] = {
					courses: yearCourses,
	...
	function getCurrentGradingSystem() {
		...
		return countryData && (countryData as any)[selectedGradingSystem]
			? (countryData as any)[selectedGradingSystem]
			: null;
	}
	...
	async function processTranscript() {
		...
		try {
			...
			if (analysisResult.totalCourses > 0) {
				...
				smartAssistData.extractedLines = extractedText
					.split('\n')
					.map((line) => line.trim())
				...
				setTimeout(() => {
					const detectedCount = smartAssistData.suggestedCourses.filter(
						(item) => item.suggested
					).length;
	...
	async function processPDFToText(file: File): Promise<string> {
		try {
			...
			throw new Error(
				`PDF processing failed: ${error.message}. Please convert your PDF to JPG using PDF24.org or SmallPDF.com and try again.`
			);
		}
	}

	// created a SvelteKit server endpoint to handle OCR requests
	...
	async function analyzeTranscriptStructure(text: string): Promise<{
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (courseData) {
				...
				coursesFound++;
				debugInfo.push(
					`Found course: ${courseData.code} - ${courseData.name} (${courseData.confidence})`
				);
			} else {
	...
	function extractCourseFromLine(line: string): {
		...
		const patterns = [
			...
			{
				regex:
					/^(SUYE\d{3,4}|[A-Z]{3,5}\d{3,4})\s+([A-Z\s&]+(?:IN|OF|FOR|TO|WITH|AS|AND)[A-Z\s&]*)\s+([A-Z]{1,2})\s+(\d+)\s+/i,
				map: (match: RegExpMatchArray) => ({
					code: match[1].trim(),
					name: match[2].trim(),
	...
	function confirmEditedCourses() {
		...
		const newCourses = coursesForEditing.map((course) => {
			...
			const usGPA =
				currentGradingSystem && gradeKey in currentGradingSystem
	...
	function extractCoursesFromText(
		text: string
	): Array<{ name: string; code?: string; grade: string; credits: number; year?: string }> {
		...
		for (const line of lines) {
			...
			if (courseData) {
				courses.push({
					...
					grade: courseData.grade,
					credits:
						typeof courseData.credits === 'string'
							? parseFloat(courseData.credits)
							: courseData.credits,
					year: undefined
				});
			}
		...
		if (extractedInfo && extractedInfo.courseName) {
			...
			setTimeout(() => {
				...
				const manualFormSection =
					document.querySelector('[data-section="manual-form"]') ||
	...
	function cancelSmartAssist() {
		...
		statusMessage =
			'Smart Assist cancelled. You can try uploading a different image or use manual entry.';
	}
</script>
...
<svelte:head>
	...
	<meta
		name="description"
		content="Free GPA converter for African universities. Convert grades from 50+ African countries to US 4.0 GPA scale. Trusted by 3000+ students worldwide."
	/>
</svelte:head>
	...
	<div class="max-w-5xl md:max-w-6xl mx-auto px-4 py-8">
		<div class="grid lg:grid-cols-2 gap-8">
			...
			<div class="bg-white rounded-xl shadow-lg p-6">
				...
				<div class="space-y-4 mb-6">
					...
					<div>
						<label for="university-name" class="block text-sm font-medium text-gray-700 mb-2"
					...
					<div>
						<label for="program-study" class="block text-sm font-medium text-gray-700 mb-2"
				...
				<div class="space-y-4 mb-6">
					...
					<div>
						<label for="grading-system" class="block text-sm font-medium text-gray-700 mb-2"
				...
				{#if activeTab === 'manual'}
					<div class="space-y-6" data-tab="manual" data-section="manual-form">
						...
						<div class="space-y-4">
							<div class="grid grid-cols-2 gap-4">
								<div>
									<label for="course-code" class="block text-sm font-medium text-gray-700 mb-2"
										>Course Code <span class="text-gray-500">(Optional)</span></label
									>
									<input
										id="course-code"
										type="text"
								...
								<div>
									<label for="academic-year" class="block text-sm font-medium text-gray-700 mb-2"
										>Academic Year <span class="text-gray-500">(Optional)</span></label
									>
									<select
										id="academic-year"
							...
							<div class="grid grid-cols-2 gap-4">
								<div>
									<label for="course-grade" class="block text-sm font-medium text-gray-700 mb-2"
										>Grade</label
									>
									<select
										id="course-grade"
									...
									>
										...
										{#each availableGrades as grade}
											<option value={grade}>
												{grade}
												{currentGradingSystem[grade]?.scoreRange
													? `(${currentGradingSystem[grade].scoreRange})`
													: ''}
											</option>
										{/each}
								...
								<div>
									<label for="course-credits" class="block text-sm font-medium text-gray-700 mb-2"
										>Credits</label
									>
									<input
										id="course-credits"
				...
				{#if activeTab === 'upload'}
					<div class="space-y-6">
						<h3 class="text-lg font-semibold text-gray-900">
							Upload Transcript for Automatic Course Extraction
						</h3>
								...
								<div class="space-y-4">
									...
									<div class="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
										<p class="text-xs text-yellow-800">
											...
											Images (JPG/PNG) provide significantly better text recognition than PDFs! If you
											have an image of your transcript, that's your best option for accurate course extraction.
										</p>
									</div>
									...
									<div class="mt-2 p-3 bg-green-50 rounded-lg border border-green-200">
										<p class="text-xs text-green-700">
											✅ <strong>Fully Automatic:</strong> Just upload your transcript and our AI will
											extract all courses, grades, and credits automatically!
										</p>
									</div>
								</div>
							{/if}
						...
						{#if showPreview && detectedSessions.length > 0}
							...
							>
								...
								<div class="mb-4 text-sm text-gray-600">
									We detected <strong>{detectedSessions.length}</strong> academic sessions. Please review
									and confirm the extracted courses:
								</div>
										...
										<div class="bg-white rounded-lg border border-purple-200 p-4">
											...
											>
												...
												<span class="text-sm font-normal text-gray-600"
													>- {session.courses.length} courses</span
												>
											</h5>
													...
													>
														<div class="flex-1">
															...
															<div class="text-sm text-gray-600">
																...
																<span
																	class="ml-2 px-2 py-1 text-xs rounded-full {course.confidence ===
																	'high'
						...
						{#if showEditingPhase && coursesForEditing.length > 0}
							<div
								class="bg-gradient-to-r from-blue-50 to-green-50 rounded-xl p-6 border border-blue-200"
							>
								<h4 class="text-xl font-bold text-gray-900 mb-4">
								...
								<div class="mb-4 p-3 bg-blue-100 rounded-lg border border-blue-200">
									<p class="text-blue-800 text-sm">
										📝 <strong>Review each course below:</strong> Edit any incorrect information, then
										confirm to add all courses to your conversion list.
									</p>
								</div>
									...
									{#each coursesForEditing as course, index}
										<div class="bg-white rounded-lg border border-gray-200 p-4">
											<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
												<div>
													<label
														for="edit-course-code-{index}"
														class="block text-xs font-medium text-gray-700 mb-1">Course Code</label
													>
													<input
												...
												<div class="md:col-span-2">
													<label
														for="edit-course-name-{index}"
														class="block text-xs font-medium text-gray-700 mb-1">Course Name</label
													>
													<input
												...
												<div>
													<label
														for="edit-course-credits-{index}"
														class="block text-xs font-medium text-gray-700 mb-1">Credits</label
													>
													<input
											...
											<div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
												<div>
													<label
														for="edit-course-grade-{index}"
														class="block text-xs font-medium text-gray-700 mb-1">Grade</label
													>
													<select
											...
											<div class="mt-2 flex items-center justify-between">
												<div class="flex items-center space-x-2">
													...
													<span
														class="px-2 py-1 text-xs rounded-full {course.confidence === 'high'
															? 'bg-green-100 text-green-800'
															: course.confidence === 'medium'
																? 'bg-yellow-100 text-yellow-800'
																: 'bg-red-100 text-red-800'}"
						...
						{#if showSmartAssist}
							<div
								class="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl p-6 border border-green-200"
							>
								<h4 class="text-xl font-bold text-gray-900 mb-4">🤖 Smart Assist Mode</h4>
								...
								<div class="mb-6">
									<h5 class="text-lg font-semibold text-gray-900 mb-3">
										📝 Extracted Text (Click lines to add courses)
									</h5>
									<p class="text-sm text-gray-600 mb-3">
										🎯 Click any line below to extract course information. I'll either Quick Add it
										or help you edit before adding.
									</p>
									<div class="bg-white p-4 rounded-lg border max-h-48 overflow-y-auto">
										{#each smartAssistData.extractedLines as line, index}
											...
											<button
												...
												onkeydown={(e) =>
													e.key === 'Enter' && !isProcessed && suggestFromLine(line, index)}
												title={isProcessed
													? 'Already processed ✅'
													: 'Click to extract course info and add to your list'}
											...
											>
												<div class="flex items-center justify-between">
													<div>
														...
														<span
															class:text-gray-800={!isProcessed}
									...
									<div class="text-xs text-gray-500 mt-2">
										💡 If a line doesn't have complete info, I'll pre-fill what I can find and let
								...
								<div class="mb-6">
									...
									<div class="space-y-2">
										{#each yearCourses as course, index}
											...
											<div
												class="flex items-start justify-between p-3 bg-blue-50 rounded-lg border border-blue-100"
											>
												<div class="flex-1 space-y-1">
													...
													<div class="grid grid-cols-3 gap-2 text-sm">
														<input
															...
															value={course.name}
															oninput={(e: any) => {
																courses[globalIndex].name = e.target.value;
																courses = [...courses];
																saveData();
															}}
														/>
														<select
															class="px-2 py-1 border rounded"
															bind:value={courses[globalIndex].grade}
															onchange={() => {
																const gd = currentGradingSystem?.[courses[globalIndex].grade];
																courses[globalIndex].usGPA = gd?.usGPA || 0;
																courses = [...courses];
																saveData();
															}}
														>
														...
														<input
															...
															oninput={(e: any) => {
																courses[globalIndex].credits = parseFloat(e.target.value) || 0;
																courses = [...courses];
																saveData();
															}}
														/>
					...
					>
						<div class="text-center mb-6">
							<h3 class="text-2xl font-bold text-gray-900 mb-2">
								🎯 Your Academic Profile Analysis
							</h3>
							<p class="text-gray-600">
								Quick insights into your academic strengths and opportunities
							</p>
						</div>
											...
											<div class="flex items-start space-x-3">
												...
												<div class="flex-1">
													<div class="flex items-center justify-between mb-1">
														...
														<span
															class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {weakness.severity ===
															'high'
																? 'bg-red-100 text-red-800'
							...
							<div class="flex flex-col md:flex-row items-center justify-between">
								<div class="mb-4 md:mb-0 md:mr-6">
									...
									<p class="text-purple-100">
										Upgrade to Professional for complete insights, personalized recommendations, and
										university competitiveness assessment.
									</p>
								</div>
		...
		<div class="mt-16 bg-gray-100 rounded-lg p-8">
			<div class="text-center mb-8">
				...
				<p class="text-gray-600">
					Free GPA conversion helping students achieve their academic dreams
				</p>
			</div>
		...
		>
			...
			<p class="text-blue-100 mb-6">
				Now that you can convert your GPA, use our comprehensive suite of AI-powered tools to create
				compelling application documents and secure funding.
			</p>
			<div class="flex flex-col sm:flex-row gap-4 justify-center">
				<a
					href="/sop"
...
{#if statusMessage && statusMessage.includes('Smart Assist Ready')}
	<div class="fixed top-4 right-4 z-50 max-w-sm">
		<div
			class="bg-white rounded-xl shadow-2xl border border-green-200 p-6 transform transition-all animate-pulse"
		>
			<div class="text-center">
