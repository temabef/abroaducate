-- 🚨 CRITICAL SECURITY FIXES for Views (CORRECTED VERSION)
...
DROP POLICY IF EXISTS "Users can only see own preferences" ON public.user_preferences;
DROP POLICY IF EXISTS "Users can only see own calendar events" ON public.calendar_events;
DROP POLICY IF EXISTS "Users can only see own reminders" ON public.reminders;

-- Create RLS policies to ensure users only see their own data
CREATE POLICY "Users can only see own preferences" ON public.user_preferences
FOR ALL TO authenticated
USING (user_id = auth.uid());

CREATE POLICY "Users can only see own calendar events" ON public.calendar_events
FOR ALL TO authenticated
USING (user_id = auth.uid());

CREATE POLICY "Users can only see own reminders" ON public.reminders
FOR ALL TO authenticated
USING (user_id = auth.uid()); 